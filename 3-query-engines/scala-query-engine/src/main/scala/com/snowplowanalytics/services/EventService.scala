...
object EventService {
  ...
  def getEvents(): List[SimpleEvent] = {
    ...
    convertDataStage1(attribsOfFourElements).toList
  }
  ...
  def convertDataStage1(myArray: Seq[Seq[awscala.dynamodbv2.Attribute]]): scala.collection.mutable.ArrayBuffer[com.snowplowanalytics.model.SimpleEvent] =  {
    var myList = scala.collection.mutable.ArrayBuffer.empty[SimpleEvent]
    for (a <- myArray) {
      val result = a.map(unpack)
      println(result)
      myList += SimpleEvent(Some(result(0).toInt),Some(result(1)),Some(result(3)),Some(10))
    }
    myList
  }
